...
def starting(window):
	...
	def execution():
		...
		if list_content.loc[list_content.index[0], "score"] < 0.001:
			...
			listbox = Listbox(window, width = 150, height = 40, selectmode = EXTENDED, yscrollcommand = scroll.set)

			scroll.config(command = listbox.yview)
			
			def access():
				webbrowser.open_new(str(listbox.get('active')[2]))

			access_button = Button(window, text = "Accès au URL", width = 30, height = 5, bg = "white", command = access)
			access_button.pack(side = RIGHT)
